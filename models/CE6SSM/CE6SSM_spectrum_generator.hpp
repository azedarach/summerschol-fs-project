...
#include "CE6SSM_two_scale_initial_guesser.hpp"
...
public:
   ...
   const CE6SSM<T>& get_model() const { return model; }
   const Problems<CE6SSM_info::NUMBER_OF_PARTICLES>& get_problems() const {
   ...
   void run(const QedQcd& oneset, const CE6SSM_input_parameters& input);
   void write_running_couplings(const std::string& filename = "CE6SSM_rge_running.dat") const;
   void write_spectrum(const std::string& filename = "CE6SSM_spectrum.dat") const;

private:
   RGFlow<T> solver;
   CE6SSM<T> model;
   CE6SSM_high_scale_constraint<T> high_scale_constraint;
   CE6SSM_susy_scale_constraint<T> susy_scale_constraint;
   CE6SSM_low_scale_constraint<T>  low_scale_constraint;
 ...
 */
template <class T>
void CE6SSM_spectrum_generator<T>::run(const QedQcd& oneset,
                                const CE6SSM_input_parameters& input)
{
   ...
   CE6SSM_convergence_tester<T> convergence_tester(&model, precision_goal);
   if (max_iterations > 0)
      convergence_tester.set_max_iterations(max_iterations);

   CE6SSM_initial_guesser<T> initial_guesser(&model, input, oneset,
                                                  low_scale_constraint,
                                                  susy_scale_constraint,
                                                  high_scale_constraint);
 ...
 */
template <class T>
void CE6SSM_spectrum_generator<T>::write_running_couplings(const std::string& filename) const
{
   CE6SSM<T> tmp_model(model);
   tmp_model.run_to(low_scale);

   CE6SSM_parameter_getter parameter_getter;
   Coupling_monitor<CE6SSM<T>, CE6SSM_parameter_getter>
 ...
 */
template <class T>
void CE6SSM_spectrum_generator<T>::write_spectrum(const std::string& filename) const
{
   CE6SSM_spectrum_plotter plotter;
