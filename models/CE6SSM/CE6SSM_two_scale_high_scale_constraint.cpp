...
#include "CE6SSM_two_scale_high_scale_constraint.hpp"
...
CE6SSM_high_scale_constraint<Two_scale>::CE6SSM_high_scale_constraint()
...
CE6SSM_high_scale_constraint<Two_scale>::CE6SSM_high_scale_constraint(const CE6SSM_input_parameters& inputPars_)
   : Constraint<Two_scale>()
   , fixed_scale(0.)
   , model(0)
...
void CE6SSM_high_scale_constraint<Two_scale>::apply()
{
   assert(model && "Error: CE6SSM_high_scale_constraint:"
          " model pointer must not be zero");

   if (std::fabs(model->get_g1()) > 3.0) {
#ifdef ENABLE_VERBOSE
      ERROR("CE6SSM_high_scale_constraint: Non-perturbative gauge "
...
double CE6SSM_high_scale_constraint<Two_scale>::get_scale() const
{
   return scale;
}

double CE6SSM_high_scale_constraint<Two_scale>::get_initial_scale_guess() const
{
   return initial_scale_guess;
}

void CE6SSM_high_scale_constraint<Two_scale>::set_model(Two_scale_model* model_)
{
   model = cast_model<CE6SSM<Two_scale> >(model_);
}

void CE6SSM_high_scale_constraint<Two_scale>::set_input_parameters(const CE6SSM_input_parameters& inputPars_)
{
   inputPars = inputPars_;
}

void CE6SSM_high_scale_constraint<Two_scale>::set_scale(double s)
{
   fixed_scale = s;
}

void CE6SSM_high_scale_constraint<Two_scale>::clear()
{
   scale = 0.;
   initial_scale_guess = 0.;
   fixed_scale = 0.;
...
void CE6SSM_high_scale_constraint<Two_scale>::initialize()
{
...
void CE6SSM_high_scale_constraint<Two_scale>::update_scale()
{
   ...
   const CE6SSM_soft_parameters beta_functions(model->calc_beta());
   ...
   if (errno == ERANGE) {
#ifdef ENABLE_VERBOSE
      ERROR("CE6SSM_high_scale_constraint<Two_scale>: Overflow error"
