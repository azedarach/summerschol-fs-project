...
public:
   Eigen::ArrayXd get_parameters(const CE6SSM<Two_scale>& model) {
      return model.get();
   }
   std::vector<std::string> get_parameter_names(const CE6SSM<Two_scale>&) const {
...
public:
   ...
   template <class T>
   void extract_spectrum(const CE6SSM<T>&);
...
template <class T>
void CE6SSM_spectrum_plotter::extract_spectrum(const CE6SSM<T>& model)
{
   spectrum.clear();
...
template <class Scalar, int M, int N>
std::valarray<double> CE6SSM_spectrum_plotter::to_valarray(const Eigen::Array<Scalar, M, N>& v)
{
