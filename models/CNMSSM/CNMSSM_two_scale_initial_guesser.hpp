...
#ifndef CNMSSM_TWO_SCALE_INITIAL_GUESSER_H
#define CNMSSM_TWO_SCALE_INITIAL_GUESSER_H
...
/**
 * @class CNMSSM_initial_guesser<Two_scale>
 * @brief initial guesser for the CNMSSM
 */

template<>
class CNMSSM_initial_guesser<Two_scale> : public Initial_guesser<Two_scale> {
public:
   CNMSSM_initial_guesser(CNMSSM<Two_scale>*,
                               const CNMSSM_input_parameters&,
                               const QedQcd&,
                               const CNMSSM_low_scale_constraint<Two_scale>&,
                               const CNMSSM_susy_scale_constraint<Two_scale>&,
                               const CNMSSM_high_scale_constraint<Two_scale>&);
   virtual ~CNMSSM_initial_guesser();
   virtual void guess(); ///< initial guess

   void set_running_precision(double p) { running_precision = p; }

private:
   CNMSSM<Two_scale>* model; ///< pointer to model class
   CNMSSM_input_parameters input_pars;
   ...
   CNMSSM_low_scale_constraint<Two_scale> low_constraint;
   CNMSSM_susy_scale_constraint<Two_scale> susy_constraint;
   CNMSSM_high_scale_constraint<Two_scale> high_constraint;
