...
public:
   Eigen::ArrayXd get_parameters(const CNMSSM<Two_scale>& model) {
      return model.get();
   }
   std::vector<std::string> get_parameter_names(const CNMSSM<Two_scale>&) const {
...
public:
   ...
   template <class T>
   void extract_spectrum(const CNMSSM<T>&);
...
template <class T>
void CNMSSM_spectrum_plotter::extract_spectrum(const CNMSSM<T>& model)
{
   spectrum.clear();
...
template <class Scalar, int M, int N>
std::valarray<double> CNMSSM_spectrum_plotter::to_valarray(const Eigen::Array<Scalar, M, N>& v)
{
