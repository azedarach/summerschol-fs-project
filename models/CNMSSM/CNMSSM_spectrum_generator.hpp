...
public:
   ...
   const CNMSSM<T>& get_model() const { return model; }
   const Problems<CNMSSM_info::NUMBER_OF_PARTICLES>& get_problems() const {
   ...
   void run(const QedQcd& oneset, const CNMSSM_input_parameters& input);
   void write_running_couplings(const std::string& filename = "CNMSSM_rge_running.dat") const;
   void write_spectrum(const std::string& filename = "CNMSSM_spectrum.dat") const;

private:
   RGFlow<T> solver;
   CNMSSM<T> model;
   CNMSSM_high_scale_constraint<T> high_scale_constraint;
   CNMSSM_susy_scale_constraint<T> susy_scale_constraint;
   CNMSSM_low_scale_constraint<T>  low_scale_constraint;
 ...
 */
template <class T>
void CNMSSM_spectrum_generator<T>::run(const QedQcd& oneset,
                                const CNMSSM_input_parameters& input)
{
   ...
   CNMSSM_convergence_tester<T> convergence_tester(&model, precision_goal);
   if (max_iterations > 0)
      convergence_tester.set_max_iterations(max_iterations);

   CNMSSM_initial_guesser<T> initial_guesser(&model, input, oneset,
                                                  low_scale_constraint,
                                                  susy_scale_constraint,
                                                  high_scale_constraint);
 ...
 */
template <class T>
void CNMSSM_spectrum_generator<T>::write_running_couplings(const std::string& filename) const
{
   CNMSSM<T> tmp_model(model);
   tmp_model.run_to(low_scale);

   CNMSSM_parameter_getter parameter_getter;
   Coupling_monitor<CNMSSM<T>, CNMSSM_parameter_getter>
 ...
 */
template <class T>
void CNMSSM_spectrum_generator<T>::write_spectrum(const std::string& filename) const
{
   CNMSSM_spectrum_plotter plotter;
